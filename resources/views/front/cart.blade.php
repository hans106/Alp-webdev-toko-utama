...
<div class="container mx-auto px-4 md:px-12 py-10">
    ...
    <div class="mb-6">
        <a href="{{ route('catalog') }}" class="inline-flex items-center gap-2 text-slate-500 hover:text-primary transition font-bold group">
    ...
    <div class="flex flex-col lg:flex-row gap-8">
        <div class="w-full lg:w-2/3">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div class="flex items-center gap-4 p-6 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition">
                    ...
                    <div class="flex items-center gap-3 bg-slate-50 rounded-full px-2 py-1 border border-slate-200">
                        <form action="{{ route('cart.update', $cart->id) }}" method="POST">
                            ...
                            <button type="submit" class="w-7 h-7 rounded-full bg-white hover:bg-slate-200 flex items-center justify-center font-bold text-slate-600 shadow-sm transition">-</button>
                        </form>
                    ...
                    <form action="{{ route('cart.destroy', $cart->id) }}" method="POST" onsubmit="return confirm('Yakin hapus barang ini?');">
                        @csrf @method('DELETE')
                        <button type="submit" class="text-slate-400 hover:text-rose-500 p-2 transition" title="Hapus">
        ...
        <div class="w-full lg:w-1/3">
            <div class="bg-white p-6 rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 sticky top-28">
                ...
                {{-- TOMBOL CHECKOUT --}}
                <a href="#" class="block w-full bg-gradient-to-r from-primary to-indigo-700 text-white text-center font-bold py-4 rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 transition transform hover:-translate-y-0.5">
    ...
    @else
        <div class="text-center py-24 bg-slate-50 rounded-3xl border border-dashed border-slate-300">
            <div class="w-20 h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" /></svg>
            </div>
            ...
            <a href="{{ route('catalog') }}" class="inline-flex items-center gap-2 bg-primary text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition">
