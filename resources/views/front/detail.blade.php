                    ...
                    <div class="mt-auto pt-8 border-t border-slate-100">
                        ...
                        @auth
                            @if(Auth::user()->role === 'customer')
                                {{-- JIKA CUSTOMER: TAMPILKAN TOMBOL BELI --}}
                                <form action="{{ route('cart.store', $product->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="w-full bg-gradient-to-r from-primary to-indigo-700 text-white font-bold py-4 px-6 rounded-xl hover:shadow-lg hover:shadow-indigo-500/30 transition transform hover:-translate-y-0.5 flex justify-center items-center gap-3 text-lg">
                            ...
                            @else
                                {{-- JIKA ADMIN: TAMPILKAN TOMBOL EDIT --}}
                                <a href="{{ route('admin.products.edit', $product->id) }}" class="block w-full bg-amber-500 text-white text-center font-bold py-4 px-6 rounded-xl hover:bg-amber-600 transition shadow-lg">
                                    Edit Produk (Mode Admin)
                                </a>
                            @endif
                        @else
                            {{-- JIKA GUEST (BELUM LOGIN): TAMPILKAN TOMBOL LOGIN --}}
                            <a href="{{ route('login') }}" class="block w-full bg-slate-800 text-white text-center font-bold py-4 px-6 rounded-xl hover:bg-slate-900 transition shadow-lg">
                                Login untuk Membeli
                            </a>
                        @endauth
